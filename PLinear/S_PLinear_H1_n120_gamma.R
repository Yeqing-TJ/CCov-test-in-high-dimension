...
q=floor(1.5*p^{0.3})
non0<-sqrt(eta/q)
beta<-c(0,rep(non0,q),rep(0,(p-q-1)))

beta1<-c(0,rep(non0,floor(q/2)),rep(0,(p-floor(q/2)-1)))
beta2<-c(0,rep(0,floor(q/2)),rep(non0,(q-floor(q/2))),rep(0,(p-q-1)))
...
for (j in 1:rep_time){
  ...
  Y<-3*X%*%beta1+exp(X%*%beta2/2)+exp(X[,1]-1)*(rgamma(n,1,1)-1)
...
save(list=ls(all=TRUE),file="S_PLinear_H1_n120_gamma.Rdata")
