rm(list=ls(all=TRUE))
#setwd('/Users/catherine/Desktop/XuKai-AOS/code')
#library(xtable)
...
for (j in 1:rep_time){
  ...
  Y=exp(X[,1]/sqrt(2))*(X%*%beta)+exp(X%*%beta/sqrt(2*q))+(rgamma(n,1,1)-1)
...
save(list=ls(all=TRUE),file="NS_NLinear_H1_n120_gamma.Rdata")
